...
func init() {
	InitializeDB()
	err := _gen_test_db.AutoMigrate(&models.Job{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&models.Job{}) fail: %s", err)
	}
}

func Test_jobQuery(t *testing.T) {
	job := newJob(_gen_test_db)
	...
	err = _do.Create(&models.Job{})
	if err != nil {
		t.Error("create item in table <jobs> fail:", err)
	}

	err = _do.Save(&models.Job{})
	if err != nil {
		t.Error("create item in table <jobs> fail:", err)
	}

	err = _do.CreateInBatches([]*models.Job{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <jobs> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*models.Job{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&models.Job{}, 0, 1)
	if err != nil {
