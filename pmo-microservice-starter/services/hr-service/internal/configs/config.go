...
func Load() *Config {
	...
	// e.g : c:\bootcamp\golang\pmo-microservice\
	configDir, err := getProjectRoot()
	if err != nil {
	...
	configPath := filepath.Join(configDir, "configs", "config."+env+".toml")
...
func getProjectRoot() (string, error) {
	// Ambil path file config.go saat ini
	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)
	// Naik sampai menemukan go.mod
	for {
		if _, err := os.Stat(filepath.Join(dir, "makefile")); err == nil {
			return dir, nil
		}
		parent := filepath.Dir(dir)
		if parent == dir { // sudah di root filesystem
			break
		}
		dir = parent
	}
	panic("tidak menemukan makefile - pastikan makefile disimpan di root-project")
}
